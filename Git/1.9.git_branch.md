...
### 删除出现问题
```
➜  analyze-web git:(master) git push origin -d cache1
warning: redirecting to https://gitlab.foundingaz.cn/founding/analyze-web.git/
error: unable to delete 'cache1': remote ref does not exist
error: failed to push some refs to 'http://gitlab.foundingaz.cn/founding/analyze-web.git'
```
原因本地仓库和远程没有同步，其实远程分支之前已经删除了，需要和远程同步一下：
```
➜  analyze-web git:(master) git remote prune origin
```
